...
const WalletPage = () => {
    ...
    const handlePurchase = async () => {
        ...
        try {
            ...
            if (isPreviewingPack && pullReward) {
                ...
                // Pass customerInfo through to finalizePurchase for secure server verification
                finalizePurchase(pkgToBuy, selectedPkg.rewards, customerInfo);
            }
        } catch (e) {
    ...
    const finalizePurchase = async (pkgToBuy, rewardsArray, customerInfo) => {
        ...
        // Extract RevenueCat identifiers to verify securely on the backend
        const appUserId = customerInfo?.originalAppUserId;
        const latestTransactionId = customerInfo?.nonSubscriptionTransactions?.[0]?.transactionIdentifier || '';
        ...
        const result = await processTransaction(
            ...
            packIdentifier,
            {
                ...
                appUserId: appUserId,          // Sent securely to backend
                transactionId: latestTransactionId // Used to verify/prevent replay attacks
            },
            userClan?.tag
        );
