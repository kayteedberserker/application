...
const MediaPlaceholder = ({ height = 250, onPress, type, thumbUrl, showPlayIcon = true }) => (
  <Pressable
    ...
    className="bg-gray-100 dark:bg-gray-900 items-center justify-center overflow-hidden rounded-2xl"
  >
    {thumbUrl ? (
      <Image
        source={{ uri: thumbUrl }}
    ...
    {showPlayIcon && (
      <View className="bg-black/40 p-5 rounded-full mb-2 border border-white/20">
        <Feather name={type === "video" ? "play" : "image"} size={32} color="white" />
      </View>
    )}
    <View className="bg-black/60 px-4 py-1 rounded-full border border-white/10">
...
// 🔹 Dedicated component for Lightbox Video
...
export default function PostCard({ post, setPosts, isFeed, hideMedia, syncing, similarPosts, isVisible = true }) {
    ...
    const fetchData = async () => {
      ...
      if (clanTag && !CLAN_CACHE[clanTag]) {
        ...
        if (storedClan) {
          ...
          setClanInfo(parsed);
          persistToStorage(`clan_cache_${clanTag}`, parsed);
        }
      }
      ...
      try {
        ...
        if (resAuthor.ok) {
          ...
          const authorData = {
            ...
            equippedGlow: data.user?.inventory?.find(i => i.category === 'GLOW' && i.isEquipped) || [],
            equippedBadges: data.user?.inventory?.filter(i => i.category === 'BADGE' && i.isEquipped) || []
          };
  ...
  // 🔹 REFACTORED MEDIA CONTENT - EVERYTHING TRIGGERS LIGHTBOX
        ...
        {count === 1 ? (
          <View className="w-full h-full">
            {mediaItems[0].type?.startsWith("video") || mediaItems[0].url.toLowerCase().includes("youtube") || mediaItems[0].url.toLowerCase().includes("tiktok") ? (
              ...
              <Pressable onPress={() => openItem(0)} className="w-full h-full">
                <Image source={{ uri: mediaItems[0].url }} className="w-full h-full" contentFit="cover" />
              </Pressable>
            )}
          </View>
        ) : count === 2 ? (
          <View className="flex-row w-full h-full gap-[2px]">
            {mediaItems.slice(0, 2).map((item, idx) => (
              <Pressable key={idx} onPress={() => openItem(idx)} className="flex-1 relative">
                <Image source={{ uri: item.type === "video" ? getVideoThumbnail(item.url) : item.url }} className="w-full h-full" contentFit="cover" />
                {item.type === "video" && (
          ...
          <View className="flex-row w-full h-full gap-[2px]">
            <Pressable onPress={() => openItem(0)} className="w-1/2 h-full relative">
              <Image source={{ uri: mediaItems[0].type === "video" ? getVideoThumbnail(mediaItems[0].url) : mediaItems[0].url }} className="w-full h-full" contentFit="cover" />
            ...
            <View className="w-1/2 h-full gap-[2px]">
              <Pressable onPress={() => openItem(1)} className="flex-1 relative">
                <Image source={{ uri: mediaItems[1].type === "video" ? getVideoThumbnail(mediaItems[1].url) : mediaItems[1].url }} className="w-full h-full" contentFit="cover" />
              ...
              <Pressable onPress={() => openItem(2)} className="flex-1 relative">
                <Image source={{ uri: mediaItems[2].type === "video" ? getVideoThumbnail(mediaItems[2].url) : mediaItems[2].url }} className="w-full h-full" contentFit="cover" />
                {count > 3 && (
                  <View className="absolute inset-0 bg-black/60 items-center justify-center">
                    <Text className="text-white text-2xl font-black">+{count - 3}</Text>
                  </View>
                )}
                {mediaItems[2].type === "video" && count <= 3 && <View className="absolute inset-0 items-center justify-center bg-black/20"><Feather name="play" size={20} color="white" /></View>}
              </Pressable>
            </View>
          </View>
        )}
    ...
    return (
      <View style={{ width: SCREEN_WIDTH, height: SCREEN_HEIGHT }}>
        <ImageZoom cropWidth={SCREEN_WIDTH} cropHeight={SCREEN_HEIGHT} imageWidth={SCREEN_WIDTH} imageHeight={SCREEN_HEIGHT} panToMove={true} pinchToZoom={true} enableSwipeDown={true} onSwipeDown={closeLightbox}>
          <Image style={{ width: SCREEN_WIDTH, height: SCREEN_HEIGHT }} source={{ uri: item.url }} resizeMode="contain" onLoadStart={() => setAssetLoading(true)} onLoadEnd={() => setAssetLoading(false)} />
        </ImageZoom>
            ...
            const CardContent = (
              <View
                style={{
                  ...
                  borderColor: equippedBorder ? 'transparent' : (isVerified ? highlightColor : THEME.border),
                }}
                className="flex-row items-center justify-between px-4 py-4 rounded-[28px] border-2 relative overflow-hidden"
              >
                {/* --- DYNAMIC SVG GRADIENT GLOW --- */}
                {equippedBg && (
                  <View className="absolute inset-0">
                    <Svg height="100%" width="100%">
                      <Defs>
                        <LinearGradient id="clanCardGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                          {/* We use low opacity stops so it tints the card without hiding the background color or text */}
                          <Stop
                            offset="0%"
                            stopColor={bgVisual.primaryColor || '#22c55e'}
                            stopOpacity={0.25}
                          />
                          <Stop
                            offset="100%"
                      ...
                      <Rect x="0" y="0" width="100%" height="100%" fill="url(#clanCardGradient)" />
                    </Svg>
                  </View>
                )}
      ...
      <Modal visible={lightbox.open} transparent animationType="fade" onRequestClose={closeLightbox}>
        <View style={{ flex: 1, backgroundColor: 'black' }}>
          <View className="flex-1 justify-center items-center">
            {mediaItems[currentAssetIndex] && renderLightboxContent(mediaItems[currentAssetIndex])}
          </View>
          ...
          {mediaItems.length > 1 && (
            <View className="absolute bottom-12 w-full items-center">
              <View className="bg-black/60 px-6 py-2 rounded-full border border-white/10">
                <Text className="text-white font-black tracking-widest uppercase text-xs">Asset {currentAssetIndex + 1} / {mediaItems.length}</Text>
              </View>
            </View>
          )}
        </View>
