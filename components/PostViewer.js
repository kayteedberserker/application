...
import {
    ...
    InteractionManager,
    View,
    AppState // 👈 Added for background/foreground detection
} from "react-native";
...
export default function PostsViewer() {
    ...
    const { data, size, setSize, isLoading, isValidating, mutate } = useSWRInfinite(getKey, fetcher, {
        refreshInterval: isOfflineMode ? 0 : 30000, 
        revalidateOnFocus: true, // 👈 Changed to true, though AppState listener is more reliable in RN
    ...
    // 🚀 NEW: App State Listener for Foreground Refetch
    useEffect(() => {
        const subscription = AppState.addEventListener("change", nextAppState => {
            if (
                ...
                console.log("App returned to foreground - Refetching Intel...");
                mutate(); // 👈 Manually trigger SWR refetch
            }
